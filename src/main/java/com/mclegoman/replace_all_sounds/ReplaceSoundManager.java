...
public class ReplaceSoundManager {
	...
	public static Identifier getSound(Identifier identifier) {
		if (!registry.isEmpty()) {
			...
			else if (registry.containsKey(Identifier.of(identifier.getNamespace(), "all"))) {
				return registry.get(Identifier.of(identifier.getNamespace(), "all"));
			}
			else if (registry.containsKey(Identifier.of("all", "all"))) {
				return registry.get(Identifier.of("all", "all"));
			}
		}
		return identifier;
	}
	public void reload() {
		...
		MinecraftClient.getInstance().getResourceManager().findResources("replace_all_sounds", identifier -> identifier.getPath().endsWith(".json")).forEach((identifier, resource) -> {
			try (InputStream stream = resource.getInputStream()) {
				...
				if (JsonHelper.getBoolean(reader, "enabled", true)) registry.put(Identifier.of(JsonHelper.getString(reader, "input")), Identifier.of(JsonHelper.getString(reader, "output")));
